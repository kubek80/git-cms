...
import { DEFAULT_CMSJS_TEMPLATE, EMPTY_CMS_DATA, SAMPLE_CMS_DATA } from '../utils/constants';
...
export default function CmsEditor({ initialData, onSave }: CmsEditorProps) {
  ...
  const [cmsData, setCmsData] = useState<CmsData>(() => {
    if (initialData) {
      // Make sure content isn't the default export message
      const contentIsDefault = initialData.content === DEFAULT_CMSJS_TEMPLATE || 
                             initialData.content.includes('// Add your CMS configuration here');
      
      return {
        ...initialData,
        content: contentIsDefault ? '' : initialData.content,
      };
    }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'content':
        return (
          <div className="p-4">
            <h2 className="text-lg font-medium mb-3 text-gray-800">Content Editor</h2>
